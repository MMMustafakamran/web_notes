...
\lstdefinelanguage{HTMLCSS}{
  ...
  morekeywords=[2]{width, height, max-width, min-width, box-sizing, border-box, auto, display, margin, padding, @media, screen, border, target, value, rows, cols, required, type, selected, disabled},
  sensitive=false,
  morecomment=[l]{<!--},
  morecomment=[s]{<!--}{-->},
  morestring=[b]",
...
\compactsection{Tables \& Formatting}
...
\compactsection{Forms \& Inputs (Exam Yield)}
\begin{lstlisting}[language=HTMLCSS]
<form>
...
\compactsection{Responsive Fundamentals}
\textbf{Viewport Meta:} Tells browser how to scale on mobile.
\begin{lstlisting}[language=HTMLCSS]
<meta name="viewport" content="width=device-width, initial-scale=1.0">
\end{lstlisting}
\textbf{Fluid Layouts:} Use \% or \texttt{relative} units.
\begin{lstlisting}[language=HTMLCSS]
* { box-sizing: border-box; } // Padding doesn't increase width
img { max-width: 100%; height: auto; } // Responsive image
\end{lstlisting}
\textbf{Media Queries:} Apply CSS based on screen width.
\begin{lstlisting}[language=HTMLCSS]
.column { width: 100%; } // Default (Mobile)
