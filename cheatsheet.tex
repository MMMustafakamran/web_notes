...
\compactsection{Detailed React Example (State, Routing, HOC)}
\begin{lstlisting}[language=JavaScript, basicstyle=\fontsize{4}{4.4}\selectfont\ttfamily]
...
class App extends Component {
  state = {
    ninjas: [ { name: 'Ryu', age: 30, id: 1 }, { name: 'Yoshi', age: 20, id: 2 } ],
    isLoading: false, userInput: ''
  };

  componentDidMount() {
    this.setState({ isLoading: true });
    axios.get('URL').then(res => { this.setState({ isLoading: false }); });
  }
  ...
  handleSubmit = (e) => {
    e.preventDefault();
    const newNinja = { name: this.state.userInput, age: Math.floor(Math.random()*40), id: Math.random() };
    this.setState({ ninjas: [...this.state.ninjas, newNinja], userInput: '' });
  };
        ...
        <div className="App">
          ...
          <Switch>
            <Route exact path="/">
              ...
              <EnhancedList isLoading={this.state.isLoading} ninjas={this.state.ninjas} deleteNinja={this.deleteNinja} />
            </Route>
            <Route path="/ninja/:id" render={(props) => ( <div>ID: {props.match.params.id}</div> )} />
          </Switch>
        </div>
      </BrowserRouter>
    );
